 ...
 * classifies again.
 */
export function reuseManifest(
  raw: string,
  sections: RouterInput[],
  /** This run's split warnings, so the carried-over set excludes them. */
  ...
  /**
   * Warnings travel with the manifest, because some of them are the manifest:
   * "5 sections could not be classified" is the difference between a trade
   * being absent and nobody having looked. Dropped on reuse, that caution
   * disappeared from the screen at the moment the user committed to a build.
   * The split ones are regenerated by this run, so they are filtered out rather
   * than shown twice.
   */
  const priorWarnings = Array.isArray(parsed?.warnings)
    ? parsed.warnings.filter(
        (w: unknown): w is string => typeof w === 'string' && !splitWarnings.includes(w),
      )
    : [];

  return { sections: routed, trades, warnings: priorWarnings };
}
